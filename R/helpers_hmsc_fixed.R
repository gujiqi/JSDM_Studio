...
safe_read_csv <- function(path, label = "file") {
  if (is.null(path) || is.na(path) || path == "" || !file.exists(path)) {
    stop(sprintf("not found %s: %s", label, path), call. = FALSE)
  }
...
mcmc_diagnostics_rich <- function(m, outdir, cfg, log_fun) {
  ...
  capture_to_file({
    ...
    cat("Description: PSRF/Gelman valuethe closer 1 the better; haseffectsamplesamountlargerthe better.\n\n")
...
parameter_estimates_rich <- function(m, outdir, cfg, log_fun) {
  ...
  capture_to_file({
    ...
    cat("including posterior meanvalue, support Pr(x>0), Pr(x<0), and availableof Beta/Gamma/Omega figures.\n")
  }, result_txt)
  ...
  }, "Beta parameter estimates", log_fun)
  ...
  }, "Gamma parameter estimates", log_fun)
  ...
  }, "Omega species associations", log_fun)
}
...
prediction_gradients_rich <- function(m, outdir, cfg, log_fun) {
  ...
  for (covariate in covariates) {
    ...
    }, paste0("predictiongradient ", covariate), log_fun)
  }
}
...
fit_hmsc_workflow <- function(Y, XData, TrData = NULL, phyloTree = NULL, studyDesign = NULL,
  ...
  if (isTRUE(cfg$outputs$compute_variance_partitioning)) {
    ...
    }, "variance partitioning RDS/CSV", log_fun)
  }
