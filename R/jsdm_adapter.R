...
write_jsdm_reproducible_script <- function(outdir) {
  ...
  script <- c(
    ...
    "    binomial_probit_long_format = { long_dat <- clean_df(long_raw); names(long_dat)[names(long_dat) == (cfg$model$long_site_col %||% 'site')] <- 'site'; names(long_dat)[names(long_dat) == (cfg$model$long_species_col %||% 'species')] <- 'species'; names(long_dat)[names(long_dat) == (cfg$model$long_response_col %||% 'presence')] <- 'Y'; long_covars <- setdiff(names(long_dat), c('site','species','Y')); sf_txt <- paste(deparse(common$site_formula), collapse=' '); needs_species_interactions <- !grepl('species', sf_txt, fixed=TRUE) || !grepl('species:', sf_txt, fixed=TRUE); if (needs_species_interactions) { rhs <- if (length(long_covars)) paste(c('species', paste0('species:', long_covars)), collapse=' + ') else 'species'; common$site_formula <- as.formula(paste('~', rhs)); append_warning(paste('long-format site_formula was rewritten for jSDM 0.2.7:', deparse(common$site_formula))) }; common$data <- long_dat; common$trait_data <- NULL; common$trait_formula <- NULL; quote(do.call(jSDM::jSDM_binomial_probit_long_format, common)) },",
