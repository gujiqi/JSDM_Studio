...
load("models/models_thin_40_samples_250_chains_4.Rdata")
load("models/models_thin_40_samples_250_chains_4_notree_withtrait/models_thin_40_samples_250_chains_4.Rdata")
names(models)
models
##################################################################################################
...
if (file.exists(filename)) {
  ...
  ## 2. ===== 默认参数初始化（只在这里）=====
  if (!exists("var.part.order.explained")) {
    var.part.order.explained <- vector("list", nm)
    for (j in 1:nm) var.part.order.explained[[j]] <- 0
  }
  
  if (!exists("var.part.order.raw")) {
    var.part.order.raw <- vector("list", nm)
    for (j in 1:nm) var.part.order.raw[[j]] <- 0
  }
  
  if (!exists("omega.order")) {
    omega.order <- vector("list", nm)
